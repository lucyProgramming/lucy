...
func (e *Expression) relationBinaryConstFolder(bin *ExpressionBinary) (is bool, err error) {
	...
	// string and string
	if bin.Left.Type == EXPRESSION_TYPE_STRING && bin.Right.Type == EXPRESSION_TYPE_STRING {
		is = true
		e.Type = EXPRESSION_TYPE_BOOL
...
func (e *Expression) relationCompare(typ int, value1, value2 interface{}) (b, support bool) {
	...
	case EXPRESSION_TYPE_BOOL:
		...
		} else if e.Type == EXPRESSION_TYPE_NE {
			b = value1.(bool) != value2.(bool)
		}
		return
	case EXPRESSION_TYPE_BYTE:
		if e.Type == EXPRESSION_TYPE_EQ {
			...
			b = value1.(byte) != value2.(byte)
		} else if e.Type == EXPRESSION_TYPE_GT {
			b = value1.(byte) > value2.(byte)
		} else if e.Type == EXPRESSION_TYPE_GE {
			b = value1.(byte) >= value2.(byte)
		} else if e.Type == EXPRESSION_TYPE_LT {
			b = value1.(byte) < value2.(byte)
		} else if e.Type == EXPRESSION_TYPE_LE {
			b = value1.(byte) <= value2.(byte)
		}
		return
	case EXPRESSION_TYPE_INT:
