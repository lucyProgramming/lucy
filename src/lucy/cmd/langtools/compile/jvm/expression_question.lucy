...
func (this *BuildExpression) buildQuestion(
	...
	maxStack = this.build(class, code, question.Selection, context, state)
	falseExit := (&cg.Exit{}).Init(cg.OP_ifeq, code)
	//true part
	stack := this.build(class, code, question.True, context, state)
	if stack > maxStack {
		maxStack = stack
	}
	...
	stack = this.build(class, code, question.False, context, state)
	if stack > maxStack {
		maxStack = stack
	}
